      ...
      <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
            <div className="max-w-2xl">
              ...
              <p className="mt-4 text-lg leading-8 text-slate-300">
                {guideLinks.length} practical, South Africa-specific guides
                that pair with our calculators — budgeting, saving,
                compounding, credit, debt, and car finance.
              </p>
            </div>

            <Link
              to="/guides"
              className="inline-flex shrink-0 items-center gap-1.5 text-sm font-medium text-[#E8C685] hover:text-[#E8C685]/80"
            >
              View all guides
              <ArrowRight size={16} />
            </Link>
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {guideLinks.map((guide, i) => (
            <Reveal key={guide.path} delay={i * 60}>
              <Link
                to={guide.path}
                className="group flex h-full flex-col justify-between rounded-[1.75rem] border border-white/10 bg-white/5 p-6 backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-[#D6A343]/30"
              >
                <div>
